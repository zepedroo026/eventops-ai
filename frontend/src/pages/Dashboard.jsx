    ...
    <div className="dashboard">
      <header className="dash-header">
        <div className="dash-brand">
          ...
          <span className="dash-brand-name">EventOps</span>
        </div>
        <div className="dash-user">
          <span className="dash-user-name">{user.nome}</span>
      ...
      <main className="dash-main">
        ...
        {!loading && error && (
          <div className="dash-state error">
            <p>{error}</p>
            <button className="btn-secondary" onClick={() => { setError(''); setLoading(true); api.get('/eventos').then(d => setEventos(d ?? [])).catch(() => setError('Erro ao carregar.')).finally(() => setLoading(false)); }}>
              Tentar novamente
            </button>
          </div>
        )}
        ...
        {!loading && !error && eventos.length === 0 && (
          <div className="dash-state">
            <p className="dash-empty">Ainda não existem eventos.</p>
          </div>
        )}
            ...
            {eventos.map(ev => (
              <article key={ev.id} className="event-card">
                <div className="event-card-top">
                  ...
                  {ev.localizacao && (
                    <span className="event-location">{ev.localizacao}</span>
                  )}
                </div>

                {ev.descricao && (
                  <p className="event-desc">{ev.descricao}</p>
                )}
