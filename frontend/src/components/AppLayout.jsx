...
export default function AppLayout() {
  ...
  const isFornecedor = user.perfil === 'Fornecedor';

  /* Redirect Staff/Fornecedor users to their own pages */
  useEffect(() => {
    if (isStaff      && window.location.pathname === '/dashboard') navigate('/staff-dashboard',     { replace: true });
    if (isFornecedor && window.location.pathname === '/dashboard') navigate('/fornecedor-portal',   { replace: true });
  }, []);
      ...
      <aside className={`sidebar${sidebarOpen ? ' sidebar-open' : ''}`}>
        <div className="sidebar-top">
          ...
          <nav className="sidebar-nav">
            {isStaff ? (
              <NavLink to="/staff-dashboard" className={({ isActive }) => 'sidebar-link' + (isActive ? ' active' : '')} onClick={closeSidebar}>
              ...
              <NavLink to="/fornecedor-portal" className={({ isActive }) => 'sidebar-link' + (isActive ? ' active' : '')} onClick={closeSidebar}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
